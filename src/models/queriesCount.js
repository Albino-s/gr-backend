...
export const getCountRecipesByWithIngredients = R.curry((httpQuery, query) => {
  let {withIngredientIds, countWithIngredients, search, time} = httpQuery;
  if (!withIngredientIds || !countWithIngredients || !time) {
    throw invalidInputError;
  }
  search = search || '';
  const rawQueryStr = `
...
export const getCountRecipesByWithoutIngredients = R.curry((httpQuery, query) => {
  let {withoutIngredientIds, search, time} = httpQuery;
  if (!withoutIngredientIds || !time) {
    throw invalidInputError;
  }
  search = search || '';
  const rawQueryStr = `
    ...
    and (recipes.name  LIKE '%${search}%' OR recipes.instructions LIKE '%${search}%'
     OR recipes.description LIKE '%${search}%')
    and (recipes.prep_time + recipes.cook_time) <= ${time}) as t1`;
  return query(builQueryObj(rawQueryStr));
});
