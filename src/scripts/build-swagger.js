...
const run = (traverse, contentBuffer, filePath) => traverse(
  yaml.safeLoad(contentBuffer.toString()),
  R.partial(resolve, [dirname(filePath)])
);

const traverse = async (swagger, swaggerPath) => {
  return await foldP(async (acc, [key, value]) => {
    if (key === '$fs-ref') {
      return await foldP(async (acc, value) => {
        const dependencyPath = swaggerPath(value);
        const content = await fs.readFile(dependencyPath);
      ...
      }, acc, R.unless(R.isArrayLike, R.of, value));
    }
